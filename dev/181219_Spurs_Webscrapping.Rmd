...
```{r}
# Clear global environment and start fresh
rm(list=ls())
...
scrape_epl_data <- function(match, path.lineups, path.events) {
  ...
  match.details$line.ups <- scrape_lineups(matchlink, path.lineups)
  
}

test <- scrape_epl_data(match = match, path.lineups = path.lineups, path.events = path.events)
```
